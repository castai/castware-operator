...
func (v *ComponentCustomValidator) ValidateCreate(ctx context.Context, obj runtime.Object) (admission.Warnings, error) {
	...
	// minimal (base) permissions, while any non-readonly mode tag (node-autoscaler,
	// workload-autoscaler, full) or an explicitly enabled cluster-controller
	// sub-component requires extendedPermissions. The webhook is deliberately
	// permissive about which tags exist — the umbrella chart itself enforces tag
	// mutual-exclusivity — so only the permission posture is gated here.
