...
func TestScanExistingComponent_AutoscalingValueCorrection_CSU5996(t *testing.T) {
	t.Run("overrides stored autoscaling.enabled=false to true when castai-workload-autoscaler is deployed", func(t *testing.T) {
		t.Parallel()
		r := require.New(t)
		ctx := context.Background()
		ctrl := gomock.NewController(t)
		...
		workloadAutoscaler := &appsv1.Deployment{
			ObjectMeta: metav1.ObjectMeta{
		...
		testOps := newClusterTestOps(t, cluster, workloadAutoscaler)
		expectGateUmbrellaNotInstalled(mockClient, testOps.mockHelm, cluster.Namespace)

		storedHelmValues := map[string]interface{}{
			"autoscaling": map[string]interface{}{"enabled": false},
			"image":       map[string]interface{}{"tag": "v0.39.3"},
		}
		testOps.mockHelm.EXPECT().GetRelease(helm.GetReleaseOptions{
			Namespace: cluster.Namespace, ReleaseName: "cluster-controller",
		}).Return(&release.Release{
			Name:   "cluster-controller",
			Chart:  &chart.Chart{Metadata: &chart.Metadata{Version: "0.92.15"}},
			Config: storedHelmValues,
			Info:   &release.Info{Status: release.StatusDeployed},
		}, nil)

		reconcile, err := testOps.sut.scanExistingComponent(ctx, mockClient, cluster, "cluster-controller", components.ComponentNameClusterController)
		r.NoError(err)
		r.True(reconcile)

		actual := &castwarev1alpha1.Component{}
		err = testOps.sut.Get(ctx, client.ObjectKey{Namespace: cluster.Namespace, Name: components.ComponentNameClusterController}, actual)
		r.NoError(err)
		r.NotNil(actual.Spec.Values)

		var values map[string]interface{}
		r.NoError(json.Unmarshal(actual.Spec.Values.Raw, &values))
		autoscaling, ok := values["autoscaling"].(map[string]interface{})
		r.True(ok, "autoscaling block must be preserved")
		r.Equal(true, autoscaling["enabled"], "stored false must be overridden to true when workload-autoscaler is deployed")

		// Sibling keys must be untouched; partial mutation of the value map
		// would be a regression in itself.
		image, ok := values["image"].(map[string]interface{})
		r.True(ok, "image block must be preserved")
		r.Equal("v0.39.3", image["tag"])
	})

	t.Run("preserves stored autoscaling.enabled=false when castai-workload-autoscaler is NOT deployed", func(t *testing.T) {
		t.Parallel()
		r := require.New(t)
		ctx := context.Background()
		ctrl := gomock.NewController(t)
		...
		// No workload-autoscaler Deployment in the fake client.
		testOps := newClusterTestOps(t, cluster)
		expectGateUmbrellaNotInstalled(mockClient, testOps.mockHelm, cluster.Namespace)
		...
		var values map[string]interface{}
		r.NoError(json.Unmarshal(actual.Spec.Values.Raw, &values))
		autoscaling := values["autoscaling"].(map[string]interface{})
		r.Equal(false, autoscaling["enabled"], "stored false must be preserved when workload-autoscaler is not deployed")
	})

	t.Run("never overrides stored autoscaling.enabled=true", func(t *testing.T) {
		t.Parallel()
		r := require.New(t)
		ctx := context.Background()
		ctrl := gomock.NewController(t)
		...
		r.NoError(json.Unmarshal(actual.Spec.Values.Raw, &values))
		autoscaling := values["autoscaling"].(map[string]interface{})
		r.Equal(true, autoscaling["enabled"], "a stored true must never be flipped to false")
	})

	t.Run("preserves stored values when autoscaling key is absent", func(t *testing.T) {
		t.Parallel()
		r := require.New(t)
		ctx := context.Background()
		ctrl := gomock.NewController(t)
		...
		storedHelmValuesJSON, err := json.Marshal(storedHelmValues)
		r.NoError(err)
		...
		err = testOps.sut.Get(ctx, client.ObjectKey{Namespace: cluster.Namespace, Name: components.ComponentNameClusterController}, actual)
		r.NoError(err)
		r.JSONEq(string(storedHelmValuesJSON), string(actual.Spec.Values.Raw),
			"values without an autoscaling key must be passed through untouched, even when workload-autoscaler is deployed")
	})

	t.Run("does not correct other components (e.g. castai-agent) even when workload-autoscaler is deployed", func(t *testing.T) {
		t.Parallel()
		r := require.New(t)
		ctx := context.Background()
		ctrl := gomock.NewController(t)
		...
		storedHelmValues := map[string]interface{}{
			"autoscaling": map[string]interface{}{"enabled": false},
			"image":       map[string]interface{}{"tag": "v0.161.1"},
		}
		testOps.mockHelm.EXPECT().GetRelease(helm.GetReleaseOptions{
			Namespace: cluster.Namespace, ReleaseName: "castai-agent",
		}).Return(&release.Release{
			Name:   "castai-agent",
			Chart:  &chart.Chart{Metadata: &chart.Metadata{Version: "0.161.1"}},
			Config: storedHelmValues,
			Info:   &release.Info{Status: release.StatusDeployed},
		}, nil)

		reconcile, err := testOps.sut.scanExistingComponent(ctx, mockClient, cluster, "castai-agent", components.ComponentNameAgent)
		r.NoError(err)
		r.True(reconcile)

		actual := &castwarev1alpha1.Component{}
		err = testOps.sut.Get(ctx, client.ObjectKey{Namespace: cluster.Namespace, Name: components.ComponentNameAgent}, actual)
		r.NoError(err)

		var values map[string]interface{}
		r.NoError(json.Unmarshal(actual.Spec.Values.Raw, &values))
		autoscaling := values["autoscaling"].(map[string]interface{})
		r.Equal(false, autoscaling["enabled"],
			"autoscaling value correction is scoped to cluster-controller only; other components must be passed through verbatim")
	})
}
